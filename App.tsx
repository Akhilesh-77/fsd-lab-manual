...
export default function App() {
  const [currentPage, setCurrentPage] = useState<PageView>('manual');
  const [pdfOpen, setPdfOpen] = useState(false);

  const ManualPage = () => (
    <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 relative overflow-hidden h-full">
       {!pdfOpen ? (
         <div className="text-center z-10 animate-fade-in max-w-2xl w-full flex flex-col items-center">
            ...
            <button 
              ...
              className="group relative inline-flex items-center justify-center px-10 py-5 text-lg font-bold text-blue-900 transition-all duration-300 bg-white rounded-full hover:scale-105 hover:shadow-[0_0_40px_-10px_rgba(255,255,255,0.7)] focus:outline-none focus:ring-4 focus:ring-white/30 shadow-xl"
            >
         ...
         <div className="w-full h-full flex flex-col animate-in fade-in duration-500">
            <div className="flex-1 bg-white rounded-2xl shadow-2xl overflow-hidden relative">
                ...
                <button 
                    ...
                    className="absolute top-4 right-4 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full backdrop-blur-sm transition-all hover:scale-110 shadow-lg z-50"
                    title="Close Viewer"
                >
                    <X size={24} />
                </button>
    ...
    <div className="h-screen w-screen bg-[conic-gradient(at_top,_var(--tw-gradient-stops))] from-slate-900 via-purple-900 to-slate-900 flex flex-col overflow-hidden text-gray-900 font-sans selection:bg-blue-500 selection:text-white">
      ...
      <nav className="h-auto md:h-24 py-4 md:py-0 flex flex-col md:flex-row items-center justify-center md:justify-between px-4 md:px-8 z-50 w-full gap-4 md:gap-0">
         ...
         <div className="flex bg-black/20 backdrop-blur-xl rounded-full p-1.5 border border-white/10 shadow-2xl overflow-x-auto max-w-full">
             <button 
                onClick={() => { setCurrentPage('manual'); setPdfOpen(false); }}
                className={`relative px-6 md:px-8 py-2.5 rounded-full text-sm font-bold transition-all duration-300 overflow-hidden whitespace-nowrap ${
                    currentPage === 'manual' 
             ...
             <button 
                onClick={() => { setCurrentPage('creators'); setPdfOpen(false); }}
                className={`relative px-6 md:px-8 py-2.5 rounded-full text-sm font-bold transition-all duration-300 flex items-center overflow-hidden whitespace-nowrap ${
             ...
             <button 
                onClick={() => { setCurrentPage('cohort-owners'); setPdfOpen(false); }}
                className={`relative px-6 md:px-8 py-2.5 rounded-full text-sm font-bold transition-all duration-300 flex items-center overflow-hidden whitespace-nowrap ${
                    currentPage === 'cohort-owners' 
                    ? 'bg-white text-slate-900 shadow-lg scale-105' 
                    : 'text-white/60 hover:text-white hover:bg-white/5'
                }`}
             >
      ...
      <main className="flex-1 flex relative w-full overflow-hidden">
         ...
         {currentPage === 'cohort-owners' && <CohortOwnersPage />}
      </main>
